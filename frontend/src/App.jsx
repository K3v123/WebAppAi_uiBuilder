...
function App() {
  ...
  const loadSavedApps = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/load-apps');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Save to MongoDB
      await axios.post('http://localhost:5000/api/save-app', {
        ...response.data,
        description
      });
  ...
  return (
    <div className="App" style={{
      ...
      <div style={{
        ...
        <h3 style={{ color: '#4a5568', marginBottom: '1rem' }}>Describe your app idea:</h3>
        <textarea
          value={description}
      ...
      {parsedData && (
        <div style={{
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            <h3 style={{ color: '#4a5568', marginBottom: '0.5rem' }}>👤 Role Switcher</h3>
          ...
          {/* Entity Forms */}
          <div>
            <h3 style={{ color: '#4a5568', marginBottom: '1rem' }}>📋 Entity Forms</h3>
            <div style={{ display: 'grid', gap: '1.5rem', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))' }}>
              {parsedData.entities.map((entity) => (
                ...
                >
                  <h4 style={{ margin: '0 0 1rem 0', color: '#2d3748' }}>{entity} Form</h4>
                  {(() => {
                    const fieldMap = {
                    ...
                    return fields.map((field, idx) => (
                      <div key={idx} style={{ marginBottom: '1rem' }}>
                        <label style={{ display: 'block', fontWeight: '500', marginBottom: '0.25rem', color: '#4a5568' }}>
